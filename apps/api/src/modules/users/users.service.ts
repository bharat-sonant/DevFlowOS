...
export class UsersService extends UsersServiceBase {
  ...
  if (!userToken || !userToken.users?.companies) {
    console.log('Either userToken is null or associated user does not exist.');
    return null;
  }
  return {
    email: userToken.email,
     companyId: userToken.users.company_id, 
  ...
  try {
    return await this.prisma.$transaction(async (tx) => {
      // 1) Load token
      const userToken = await tx.user_tokens.findFirst({
        where: { 
          token: dto.token,
          is_verified: false,
          used_at: null,
      ...
      const existing = await tx.users.findFirst({
        where: { company_id: companyId, username: dto.username },
        select: { id: true },
      });
      if (existing) {
        throw new ConflictException('Username already exists within the company');
  ...
  userRecord.email,
  userRecord.is_owner!,
  false // rememberMe false by default
);
